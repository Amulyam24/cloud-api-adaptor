...
func (b *ByomProvisioner) CreateCluster(ctx context.Context, cfg *envconf.Config) error {
	...
	cmd := exec.Command("docker", "exec", "peer-pods-worker", "sed", "-i",
	...
	if err != nil {
		...
		cmd = exec.Command("docker", "exec", "peer-pods-worker", "systemctl", "restart", "containerd")
		output, err = cmd.CombinedOutput()
		if err != nil {
			...
			cmd = exec.Command("docker", "exec", "peer-pods-worker", "systemctl", "is-active", "containerd")
			output, err = cmd.CombinedOutput()
...
func (b *ByomInstallChart) Install(ctx context.Context, cfg *envconf.Config) error {
	...
	for time.Now().Before(timeout) {
		checkLabel := exec.Command("kubectl", "get", "node", "peer-pods-worker", "--show-labels")
