...
func NewProvider(config *Config) (provider.Provider, error) {
	...
	if config.PoolSize > 0 {
		...
		if err := pvsProvider.initializePodVMPool(context.Background(), config.PoolSize); err != nil {
			return nil, err
		}
...
// initializePodVMPool created a new pool of pre-created VMs.
func (p *ibmcloudPowerVSProvider) initializePodVMPool(ctx context.Context, poolSize int) error {
...
// checkPodVMPoolSize monitors and updated the podVM pool with the desired number of instances.
func (p *ibmcloudPowerVSProvider) checkPodVMPoolSize(ctx context.Context, poolSize int) {
	...
	for {
		...
		for i, instance := range *p.serviceConfig.PreCreatedInstances {
			...
			if err != nil {
				if strings.Contains(err.Error(), "pvm-instance does not exist") {
					logger.Printf("pod VM does not exist the pool %s", instance.Name)
		...
		if count < poolSize {
			...
			logger.Printf("Updating pool with adding %d pod VMs", count)
